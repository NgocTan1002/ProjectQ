...
CREATE OR REPLACE FUNCTION public.fn_get_customer_orders(
    ...
    RETURNS TABLE(order_id uuid, order_number text, order_type text, status text, total numeric, currency character varying, item_count bigint, created_at timestamp with time zone) 
    LANGUAGE 'sql'
    COST 100
